...
class SeparableConv1d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=1, stride=1, dilation=1, bias=True):
        ...
        self.conv1 = nn.Conv1d(in_channels, in_channels, kernel_size, stride, kernel_size // 2, dilation,
...
class TS_VAD_MC(nn.Module):
    ...
    def forward(self, x, embedding, nframes):
        ...
        embedding_reshape = embedding.repeat(1, Channel, 1).reshape(-1, embedding_dim)[..., None].expand(
            batchsize * Channel * speaker, embedding_dim, Time)
        ...
        x_12 = torch.mean(
            x_11.reshape(batchsize, Channel, speaker, Time, self.cnn_attention[1]).transpose(1, 3).transpose(1, 2), -2)
...
class TS_VAD_SC(nn.Module):
    ...
    def forward(self, x, embedding, nframes):
        ...
        embedding_reshape = embedding.reshape(-1, embedding_dim)[..., None].expand(batchsize * speaker, embedding_dim,
                                                                                   Time)
        # print(embedding_reshape.shape)
        x_7 = torch.cat((x_6_reshape, embedding_reshape), dim=1)
        '''
