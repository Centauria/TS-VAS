...
class Train_SingleGPU():
    def __init__(self, data_sets, collate_fn, model_func, model_configs, model_name, output_dir, optimizer, criterion, batchsize=16, accumulation_steps=1, clip=-1, lr=0.001, start_epoch=0, end_epoch=10, cuda=-1, num_workers=16):
    ...
    def train(self):
        ...
        for t in range(self.start_epoch, self.end_epoch): 
            ...
            chunk_loss = 0.0
            self.nnet.train()
            pbar = tqdm(enumerate(prefetch_generator.BackgroundGenerator(self.data_loader)), total=len(self.data_loader))
            #point = time.time()
            for i, batch_data in pbar:
                ...
                if ((i + 1) % (1000 * accumulation_steps) == 0):
                    utils.save_checkpoint(self.nnet, self.optimizer, "{}/{}.model{}_{}".format(self.output_dir, self.model_name, t, (i + 1) // accumulation_steps))
            ...
            utils.save_checkpoint(self.nnet, self.optimizer, "{}/{}.model{}".format(self.output_dir, self.model_name, t + 1))
